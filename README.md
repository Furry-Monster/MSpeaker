...
{{DialogueName(ChooseAlice)}}
{{SetVariable("playerName", "Alice")}}

[Speaker]
很高兴认识你，Alice！

{{DialogueName(ChooseBob)}}
{{SetVariable("playerName", "Bob")}}
...
```
{{DialogueName(StartScene)}}
# 或
{{ConversationName(StartScene)}}
```
...
```
{{SetVariable("playerName", "Alice")}}      # 设置变量
{{AddAffection("shikiAffection", 5)}}       # 增加数值（自定义函数示例）
你的名字是：$playerName                      # 使用变量
```
...
```
- 选项 -> DialogueName ## affection:5 flag
```
...
{{Goto(CheckPoint)}}  # 跳转到CheckPoint标签
```
...
```
[Speaker]
对话内容 ## key1:value1 key2:value2 flag
```

元数据用于向对话行添加额外信息，可在运行时访问。
